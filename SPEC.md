...
2.  **YouTube動画字幕取得:** 入力されたURLの動画から`youtube-transcript-api`を使用して字幕テキストを取得する。
...
*   **目的:** YouTube URLを受け取り、動画から字幕を取得する。また、動画タイトル・チャンネル名・サムネイルURLなどのメタデータも取得する。
*   **入力 (A2A Task):** `message.parts` に `youtube_url` を含む `DataPart` または `TextPart`。
*   **処理フロー:**
    1.  URL抽出・検証。
    2.  `yt-dlp`を使用してビデオメタデータ（動画タイトル、チャンネル名、サムネイルURL）を取得。
    3.  字幕取得フロー:
        a. `youtube-transcript-api`を使用して手動追加された字幕または自動生成字幕を取得。
        b. 優先言語順（デフォルト: ["ja", "en"]）で字幕を検索。
        c. 字幕が取得できない場合はエラーを返す。
    4.  メタデータ（動画タイトル、チャンネル名、サムネイルURL）と字幕テキストを次のエージェントに転送。
*   **出力 (A2A Task):**
...
*   **現状:** Flaskで実装済み。Dockerで実行。字幕取得に`youtube-transcript-api 1.x`を使用。ビデオメタデータ（動画タイトル・チャンネル名・サムネイルURL）の取得と連携を強化。タスク状態をメモリ管理し`/tasks/get`が実際のステータスを返すよう実装。エラーの上流伝播を改善。
*   **今後の課題:** 非同期処理導入、エラーハンドリングのさらなる強化。
...
*   YouTube動画処理エージェントの字幕取得を`youtube-transcript-api 1.x`に移行し、レート制限問題を解消
