...
*   **認証:** Flaskセッション（署名付きCookie）によるフォームログイン認証。IPベースのログイン試行ロックアウト付き
...
*   **現状:** 字幕取得に`youtube-transcript-api 1.x`を使用。動画タイトルを含むメタデータの取得・連携を実装。タスク状態をメモリ管理し`/tasks/get`が実際のステータスを返すよう実装。下流エージェントの失敗を正しく上流に伝播するよう修正。ヘルスチェックを実装。下流エージェントへの転送がタイムアウトした場合は通信エラーと区別し、専用の日本語メッセージを表示する。
...
*   **処理フロー:**
    ...
    2.  Gemini API (`gemini-flash-latest`, JSONモード) にプロンプトを送信してレシピ情報を抽出。
        ...
        *   429エラーのうち1分あたりの上限（RPM）は待機して自動リトライ。リトライ上限到達時・日次上限（RPD）超過時は、ユーザー向けの日本語メッセージ（例:「1日のGemini API無料枠の上限に達しました。翌日以降に再試行してください。」）を生成して即座に失敗させる。
        *   APIキー未設定・AI応答のJSON解析失敗・空応答も、それぞれ専用の日本語エラーメッセージとして`RuntimeError`で上流に伝播する。
...
*   **現状:** `gemini-flash-latest`を使用して常に最新のFlashモデルを自動選択。動画タイトルをプロンプトに含め料理名抽出精度を向上。RPM/RPDを区別したリトライ処理を実装。Gemini API由来の各種失敗（RPM上限・APIキー未設定・JSON解析失敗・空応答）をユーザー向け日本語メッセージとして表示できるよう改善。Notion登録失敗を適切にエラーとして伝播するよう修正。
...
*   **エンドポイント:** `/.well-known/agent.json`, `/tasks/send`, `/health`
*   **現状:** A2Aプロトコルに準拠し失敗時もHTTP 200で`status: "failed"`を返すよう修正（HTTP 500から変更）。サムネイル画像をページ本文に表示する機能を実装。Notion APIエラーをコード別（認証失敗・DB未検出・プロパティ不一致・レート制限）にユーザー向け日本語メッセージへマッピングして上流へ伝播。接続情報未設定も明示的なエラーとして扱う。

### 3.4. フロントエンドUI / メインコントローラー
*   **目的:** ユーザーインターフェースを提供し、YouTube動画処理エージェントへのタスク送信・状態ポーリング・結果表示を担当する。あわせてログイン認証（フォーム＋セッション）を担う。
*   **認証フロー:**
    1.  未認証で保護ページにアクセスすると`/login`へリダイレクト。
    2.  HTMLログインフォーム（`autocomplete="username"` / `current-password`属性付き、モバイルのパスワード自動入力に対応）で資格情報を送信。
    3.  `AUTH_USERNAME` / `AUTH_PASSWORD`と一致すればセッション（署名付きCookie）を発行しトップへリダイレクト。
    4.  ログイン失敗が規定回数（既定5回）に達したIPは一定時間（既定15分）ロックアウト。
    5.  保護対象のAPI（`/submit`・`/status`）は未認証時にHTTP 401 (JSON)を返し、フロントJSがエラー表示できるようにする。
*   **処理フロー:**
...
*   **エンドポイント:** `/login` (GET/POST), `/logout` (GET), `/` (GET, 要認証), `/submit` (POST, 要認証), `/status/<task_id>` (GET, 要認証), `/health`
*   **セッション設定:** 署名鍵は環境変数`SECRET_KEY`。Cookieは`HttpOnly` / `SameSite=Lax` / `Secure`（HTTPS前提、環境変数`SESSION_COOKIE_SECURE`で無効化可）/ 有効期限30日。
*   **現状:** 認証をBasic認証からHTMLフォーム＋セッションCookieへ刷新し、モバイル（iOS Safari等）のパスワード自動入力に対応。ログイン試行ロックアウトの「失敗カウントが蓄積されず規定回数で発動しない」既存バグを修正。エラー表示を適切に実装（タイムアウト・エラー回数による誤った完了判定を削除）。Notionリンクの重複表示バグを修正（実行のたびに蓄積される問題を解消）。
...
*   エラーが発生した場合、`status: "failed"` と**ユーザー向けの日本語エラーメッセージ**が上流エージェントへ伝播し、最終的にフロントエンドのエラー表示に反映される。各エージェントは想定される失敗（API上限・タイムアウト・設定不備など）を具体的な日本語メッセージに変換して返す。
...
*   ログイン認証情報 (`AUTH_USERNAME` / `AUTH_PASSWORD`) とセッション署名鍵 (`SECRET_KEY`)
...
*   APIキー・認証情報・セッション署名鍵（`SECRET_KEY`）は環境変数として設定し、コードに直接記述しない（`.env` は `.gitignore` 対象）。
*   ユーザー入力（URL）はバックエンドで検証・サニタイズする。
*   アクセスはフォームログイン認証（Flaskセッション）で保護する。セッションCookieは `HttpOnly` / `SameSite=Lax` / `Secure` を付与。
*   総当たり対策として、IP単位でログイン失敗回数を数え、規定回数を超えたIPを一定時間ロックアウトする。
*   外部公開する場合はHTTPS化を前提とする（リバーストンネル等でTLS終端し、フォーム認証で保護する）。

---

## 9. 将来的な拡張

*   **認証の強化:** 現状はフォームログイン（単一ユーザー、Flaskセッション）。将来的にOAuth/SSOやユーザーごとのアカウント管理を追加する。
